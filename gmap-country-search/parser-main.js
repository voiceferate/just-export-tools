...
log(places.makeQueriesArray(arguments))
...
async function parseLink(url) {
  ...
  await axios(url)
    .then(response => {
      ...
      resp = resp + response.data.toString();
...
function handleResponseData(data) {
  ...
  let infoStr = data.slice(infoStrStart + 15, infoStrEnd);

  infoStr = infoStr.split(' · ')
  
  log.warn('name,', infoStr[0]);
  log.warn('address,', infoStr[1]);
  log.warn('activity,', activity)
  ...
  log.warn('phone,', currentLine.phone)
